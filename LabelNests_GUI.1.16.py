...
class LabelNestsApp(QtWidgets.QWidget):
        ...
        if not use_python_module and not labelme_executable:
            QMessageBox.critical(
                ...
                (
                    ...
                    "Install with apt (recommended on Debian/Pi):\n"
                    "  sudo apt install labelme\n\n"
                    "or with pip in your active environment:\n"
                    "  python3 -m pip install labelme"
                ),
            )
            self.labelme_process = None
            return
    ...
    def _on_labelme_process_error(self, process_error):
        if process_error == QProcess.FailedToStart:
            QMessageBox.critical(
                ...
                "LabelMe process failed to start. Check that LabelMe and PyQt are installed correctly.",
            )
    ...
    def convert_json_to_csv(self):
        ...
        try:
            jsons = sorted(glob.glob(os.path.join(self.folder_path, '*.json')))
        except Exception as exc:
            print(f"WARNING: an error was hit when trying to find the json files to convert to CSV: {exc}")
            return
        ...
        try:
            ...
            for file in jsons: #to do: check whether a CSV already exists? Or just overwrite the old one, depending on how long it takes?
                ...
                with open(csv_path, 'w', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow(["object index","label","label ID", "vertex ID","shape","x", "y", "radius"])
                    with open(file, 'r') as json_file:
                        nest = json.load(json_file)
                    object_index = -1
                    for shape in nest.get('shapes', []):
                        object_index += 1
                        label = shape.get('label')
                        ...
                        if shape_type == "circle":
                            if len(points) < 2:
                                continue
                            x_center = points[0][0] #x coordinate of point defining center of circle
                            y_center = points[0][1] #y coordinate of point defining center of circle
                            x_perimeter = points[1][0] #x coordinate of point defining the perimeter of the cirlce
                            y_perimeter = points[1][1] #y coordinate of point defining the perimeter of the cirlce

                            radius = ((x_center-x_perimeter)**2 + (y_center-y_perimeter)**2) ** 0.5
                            writer.writerow([object_index, label, label_id, 1, shape_type, x_center, y_center, radius])
                        ...
                        if shape_type == "point":
                            ...
                            x = points[0][0]
                            y = points[0][1]
                            writer.writerow([object_index, label, label_id, 1, shape_type, x, y, np.nan])
                        ...
                        if shape_type == "polygon":
                            ...
                            for point in points:
                                ...
                                x = point[0]
                                y = point[1]
                                writer.writerow([object_index,label, label_id, point_count, shape_type, x, y, np.nan])
