...
def label_single_image_with_preload(image_path):
    ...
    if idx > 0:
        ...
        if os.path.exists(prev_json) and not os.path.exists(curr_json):
            ...
            # load the current image into a byte format that labelme wants it input as
            loaded_image_data = labelme.LabelFile.load_image_file(image_path)
            image_data_utf = base64.b64encode(loaded_image_data).decode('utf-8')
            ...
            with open(curr_json, 'r+') as f:
                ...
                data['imageData'] = image_data_utf #data.pop('imageData', None)  # this removes the key entirely
                f.seek(0)
